...
If the JWT token is valid, it is stored in a [variable](/reference/variables.md), which can then be used in the flow.
...
* `alg` - The signing algorithm the JWS is expected to be created with. This can either be specified with a value, or by referencing a file (`file`) or an environment variable (`env`). See the [`algorithm` parameter for `json-decode()`](/reference/functions/jwt-decode.md) for more information.
...
The algorithm is read from the `FLAT_JWT_ALG` environment variable.
The JWT will be stored in `$jott1`.
...
The algorithm is specified as `HS256`.
The JWT will be stored in `$jott2`.
...
Swagger allows for combinations of security schemes to be applied to API routes.
If security schemes are specified as _alternatives_, at least one alternative must not fail.
In the follwing example, a `GET` request to `/foo` must satisfy at least one of the security schemes named `JWTHeaderAuth` and `JWTCookieAuth`.
...
in Swagger Security Schemes can only be specified at the top level (default security) or for specific operations.

In the following example, a `GET` request to `foo` must satisfy the security scheme named `JWTHeaderAuth`.
All other request must satisfy either the `JWTHeaderAuth` **or** `JWTCookieAuth` security schemes.
...
security:                     # alternatives: token in Authorization header or authtoken cookie
  ...
  - JWTCookieAuth: []
paths:
  /:                          # default security as defined top-level
  /foo:
    get:
